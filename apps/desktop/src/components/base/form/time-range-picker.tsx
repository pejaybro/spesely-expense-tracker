  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (is12Hour) {
      ...
      if (isDeleting) {
         ...
         if (p2.trim().endsWith("A") || p2.trim().endsWith("P")) ampm2 = "";
      }
        ...
        {isTypeable ? (
          <div className="relative flex items-center">
            <input
              ...
              placeholder={isFloating ? "" : (is12Hour ? (showSeconds ? "hh:mm:ss aa to hh:mm:ss aa" : "hh:mm aa to hh:mm aa") : (showSeconds ? "HH:mm:ss to HH:mm:ss" : "HH:mm to HH:mm"))}
              className={cn(
            ...
            />
            <Clock size={16} className={cn("absolute left-2.5 transition-colors", error ? "text-red-400" : "text-gray-400")} />
          </div>
        ) : (
          <button
          ...
          >
            ...
            <span className={cn("text-md flex-1 text-left truncate", !range.from && "text-gray-600")}>
          ...
          <FloatingPortal>
            <FloatingFocusManager context={context} modal={false}>
              <div
                ...
                {...getFloatingProps()}
                className="z-[9999] flex flex-col min-w-[340px] bg-black border border-gray-800 rounded-3xl shadow-2xl animate-in fade-in duration-200 overflow-hidden"
              >
                ...
                <div className="flex p-4 justify-center gap-1 min-h-[220px] max-h-[300px]">
                  ...
                  <div className="flex flex-col overflow-y-auto custom-scrollbar px-1 flex-1">
                     <div className="text-[10px] font-black tracking-widest text-gray-500 uppercase p-2 sticky top-0 bg-black text-center">Hour</div>
                     {hours.map(h => (
                       <button
                         key={h}
                  ...
                  {/* Minute Column */}
                  <div className="flex flex-col overflow-y-auto custom-scrollbar px-1 border-l border-gray-800 flex-1">
                     <div className="text-[10px] font-black tracking-widest text-gray-500 uppercase p-2 sticky top-0 bg-black text-center">Min</div>
                     {minutes.map(m => (
                       <button
                         key={m}
                  ...
                  {showSeconds && (
                    <div className="flex flex-col overflow-y-auto custom-scrollbar px-1 border-l border-gray-800 flex-1">
                      <div className="text-[10px] font-black tracking-widest text-gray-500 uppercase p-2 sticky top-0 bg-black text-center">Sec</div>
                      {secs.map(s => (
                        <button
                          key={s}
                  ...
                  {/* AM/PM Column */}
                  {is12Hour && (
                    <div className="flex flex-col px-1 border-l border-gray-800 flex-1">
                      <div className="text-[10px] font-black tracking-widest text-gray-500 uppercase p-2 text-center">Per</div>
                      {["AM", "PM"].map(p => (
                        <button
                          key={p}
                          onClick={() => handleTimeSelect(currentHour || 12, currentMinute || 0, currentSecond || 0, p)}
                          className={cn(
                            "w-12 h-10 shrink-0 flex items-center justify-center rounded-lg text-[11px] font-black tracking-widest transition-all cursor-pointer mx-auto",
